...
def prompt(message: str, allow_empty: bool = False, method: Callable = Prompt.ask, **kwargs) -> str | int:
  """
...
def get_ctf_config():
  ...
  while True:
    ...
    config_file = askopenfilename(
      ...
      filetypes=[("TOML files", "*.toml")] # Still a bit of hardcoding here, but should be fine
    )
...
def get_solution_files():
  ...
  """
  console.rule(":file_folder: [bright_yellow]Please select the solution files for the challenge.[/] :file_folder:")
...
def get_flags() -> list[dict[str, str | bool]]:
  ...
  """
  console.rule(":triangular_flag: [bright_yellow]Challenge Flags[/] :triangular_flag:")

  flags = []
  while True:
    regex = Confirm.ask(":triangular_flag: [cyan]Is the flag a regex flag?[/]")
    case_sensitive = Confirm.ask(":triangular_flag: [cyan]Is the flag case-sensitive?[/]")
    ...
    if len(hints) > 0:
      if Confirm.ask(":light_bulb: [cyan]Do you want to add a hint requirement?[/]"):
        ...
        for i, hint in enumerate(hints):
          console.print(f"  [green]{i}. {hint['content']} ({hint['cost']})[/]")

        valid = False
        while not valid:
          requirements = prompt(":light_bulb: [cyan]Please enter the hint requirements (comma-separated)[/]")
          requirements = [int(req) for req in requirements.split(",")]

          for req in requirements:
            if req >= len(hints):
              console.print(f"[bright_red]Invalid hint requirement: {req}[/]")
              break
          else:
            valid = True
...
def get_files() -> list[Path | str] | None:
  ...
  if not Confirm.ask(":file_folder: [cyan]Does the challenge have files to give to players?[/]"):
    return None
...
def get_services() -> list[dict[str, str | int | dict[str, str | int]]] | None:
  ...
  while True:
    ...
    port = prompt(":gear: [cyan]Please enter the service port[/]", method=IntPrompt.ask)
    service_type = prompt(":gear: [cyan]Please enter the service type[/]", choices=["web", "nc", "ssh", "secret", "internal"])
...
def create():
  ...
  for i, difficulty in enumerate(difficulties):
    console.print(f"[bright_cyan]{i+1}. {difficulty.name.capitalize()} ({difficulty.value})[/]")
  ...
  if Confirm.ask(":rocket: [cyan]Does the service(s) need a Docker Compose file?[/]"):
    ...
    if docker_compose == "":
      console.print("[bright_red]No Docker Compose file selected, aborting...[/bright_red]")
      return
    else:
