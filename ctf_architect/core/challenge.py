...
from ctf_architect.core.constants import (CHALL_README_TEMPLATE,
                                          CHALLENGE_CONFIG_FILE, CTF_CONFIG_FILE)
...
def verify_challenge_config(challenge: Challenge, config: CTFConfig | None = None) -> None:
  if config is None:
    config = load_config()
...
def add_challenge(folder: str | Path, replace: bool = False, verify: bool = True) -> None:
  """
  ...
  if challenge.full_path.exists():
    ...
    if old_challenge.name == challenge.name:
      if replace:
        ...
        raise FileExistsError(f"Challenge with name {challenge.name} already exists")
    else:
...
def walk_challenges(category: str | None = None, *, verify: bool = False) -> Generator[Challenge]:
  """
...
def create_challenge_readme(challenge: Challenge):
  ...
  extras = "\n".join(
    ...
    for key, value in challenge.extras.items()
  )
