...
CTFA_VERSION = importlib.metadata.version("ctf-architect")
...
WORKFLOWS_SPEC_VERSION = Version(MAJOR=0, MINOR=1)
...
def is_supported_port_mapping_version(version: str) -> bool:
    """Check if the specified Port Mapping version is supported by the current version of CTF Architect."""
    segments = version.split(".")
...
def is_supported_workflows_config_version(version: str) -> bool:
    """Check if the specified Workflows Config version is supported by the current version of CTF Architect."""
    segments = version.split(".")
    ...
    return int(major) == WORKFLOWS_SPEC_VERSION.MAJOR and int(minor) <= WORKFLOWS_SPEC_VERSION.MINOR
